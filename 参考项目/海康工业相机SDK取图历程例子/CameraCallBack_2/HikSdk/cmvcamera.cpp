...
int CMvCamera::ReadBuffer(cv::Mat &image)
{
    ...
    nRet = MV_CC_GetOneFrameTimeout(m_hDevHandle,pDate,nRecvBufSize,&stImageInfo,1000);
    if(MV_OK != nRet)
    {
        return nRet;
    }
...
int CMvCamera::SaveBuffer(QByteArray imageName)
{
    ...
    MV_FRAME_OUT_INFO_EX stImageInfo;
    memset(&stImageInfo,0,sizeof(MV_FRAME_OUT_INFO));
    nRet = MV_CC_GetOneFrameTimeout(m_hDevHandle,pDate,nRecvBufSize,&stImageInfo,1000);   //采用超时机制获取一帧图片
    if(MV_OK != nRet)
    {
        return nRet;
    }
