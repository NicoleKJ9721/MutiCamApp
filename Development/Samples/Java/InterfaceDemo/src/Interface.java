...
public class Interface
{
    ...
    public static int chooseEnumInterfaceType()
	{
		...
		while (true)
        {
            try
            {
                ...
                if ((EnumInterfaceTypeIndex >= 0 && EnumInterfaceTypeIndex < 3) || -1 == EnumInterfaceTypeIndex)
                {
                    break;
                }
                else
    ...
    public static int chooseInterface(ArrayList<MV_INTERFACE_INFO> stInterfaceList)
    {
        ...
        if (0 <= InterfaceIndex && stInterfaceList.size() > InterfaceIndex)
        {
            ...
            {
                System.out.println("Connect to camera[" + InterfaceIndex + "]: " + stInterfaceList.get(InterfaceIndex).serialNumber);
            }
