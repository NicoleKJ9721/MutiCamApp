...
namespace cv_dnn_nms {
	...
	vector<Match> KcgMatch::Matching(Mat source, float score_thresh, float overlap,
		...
		auto end_pyramid = std::chrono::high_resolution_clock::now();
		auto duration_pyramid = std::chrono::duration_cast<std::chrono::milliseconds>(end_pyramid - start_pyramid);
		std::cout << "    [Profile] GetAllPyramidLevelValidSource took: " << duration_pyramid.count() << " ms" << std::endl;
		...
		auto duration_matching = std::chrono::duration_cast<std::chrono::milliseconds>(end_matching - start_matching);
		std::cout << "    [Profile] Initial MatchingPyrd8 took: " << duration_matching.count() << " ms" << std::endl;
		
		matches = GetTopKMatches(matches);

		auto start_reconfirm = std::chrono::high_resolution_clock::now();
		matches = ReconfirmMatches(matches, pyrd_level);
		auto end_reconfirm = std::chrono::high_resolution_clock::now();
		auto duration_reconfirm = std::chrono::duration_cast<std::chrono::milliseconds>(end_reconfirm - start_reconfirm);
		std::cout << "    [Profile] ReconfirmMatches took: " << duration_reconfirm.count() << " ms" << std::endl;
...
#pragma omp parallel for
	...
	Template KcgMatch::SelectScatteredFeatures(vector<Candidate> candidates, int num_features, float distance) {
		...
		while (templ.features.size() < num_features) {
			...
			bool keep = true;
			for (size_t j = 0; (j < templ.features.size()) && keep; ++j)
			{
	...
	void KcgMatch::SaveModel() {
		...
		cout << "[Debug] Attempting to save model..." << endl;
		int total_templ = 0;
		for (int i = 0; i < PyramidLevel_TabooUse; i++) {
			total_templ += (int)templ_all_[i].size();
		}
		cout << "[Debug] Total templates created: " << total_templ << endl;

		if (total_templ == 0) {
			cerr << "[Error] No templates were generated. Aborting model creation. Check your creation parameters (e.g., thresholds) and template image." << endl;
			return;
		}
		...
		cout << "[Debug] Saving model to path: " << model_name << endl;

		FileStorage fs(model_name, FileStorage::WRITE);
		if (!fs.isOpened()) {
			cerr << "[Error] Failed to open FileStorage for writing. Check path and permissions." << endl;
			return;
		}
					...
					<< "[";
				{
					for (int j = 0; j < PyramidLevel_TabooUse; j++) {
						...
						fs << "features"
							<< "[";
						{
							for (size_t k = 0; k < templ.features.size(); k++) {
				...
				for (int r = 0; r < quantized_angle.rows - templ.h; r++) {
					...
					for (int c = 0; c < quantized_angle.cols - templ.w; c++) {
						...
						for (int f = 0; f < fsz; f++) {
							...
							if (partial_sum + (fsz - f - 1) < score_thresh_ * fsz) {
